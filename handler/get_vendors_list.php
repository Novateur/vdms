<?php
	...
	$sql = "SELECT * FROM vendors WHERE company_id={$id} LIMIT $start, $limit";
	...
	if($query->rowCount()>0)
	{
		...
		foreach($query as $r)
		{
			...
			<td><button type='button' class='btn btn-default btn-sm dropdown-toggle' data-toggle='dropdown'><i class='i i-bars3'></i></button>
                            <ul class='dropdown-menu'>
                              <li><a href='#' onclick=\"del_vendor({$r['id']},'list','seq_list_asc',{$page})\"><i class='i i-trashcan'></i> Delete</a></li>
                            </ul>
						</td>
			</tr>";
		}
