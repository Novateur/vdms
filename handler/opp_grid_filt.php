<?php
	...
	if($query->rowCount()>0)
	{
		...
		echo"<button type='button' style='margin-top:-15px;margin-left:0px' class='btn btn-success btn-sm dropdown-toggle' data-toggle='dropdown' id='choose_task'>choose task <i class='i i-arrow-down4'></i></button><br/>
				<ul class='dropdown-menu'>
					<span class='arrow top'></span>
					<li><a href='#' onclick=\"get_opp_grid()\"><i class='i i-clip'></i> View all tenders</a></li>
						...
						echo "<div class='col-md-3 col-sm-6'>
							<div class='panel b-a' style='width:230px'>
								<div class='panel-heading no-border bg-default lt' style='height:90px'>
									<p style='color:#5bc0de'><a href='oppurtunity_details.php?opp_ref={$r['id']}' style='color:#5bc0de'><i class='i i-clip'></i> {$r['name']}</a></p> 
			...
			echo"<div class='row'>";
				...
				echo "<div align='center'id='paginate1'>";
					...
					if($total_num_pages>1)
					{
						...
						{
							echo "<button type='button'class='btn btn-success btn-sm' onclick=\"paginate_filt({$next},{$page},{$type})\">Next</button>";
						}
					}
				echo "</div>";
				echo "</div>";
