				...
				<div class="modal-dialog" style="width:500px;">
					<div class="modal-content">
						...
						<div class="modal-body" style='font-size:14px'>
							You have successfully updated your company's information/details click the <b>OK</b> button to proceed
						</div>
						<div class="modal-footer">
							<a href='settings.php' data-role='button' class="btn btn-success">OK</a>
						</div>
					</div>
				</div>
			</div>
  ...
  <script>
	$('#edit_form').submit(function(e){
		...
		if(com_name=="" || country=="")
		{
			if(com_name=="")
			{
				$('#comp_name_msg').html("Your company name is required");
			}
			if(country=="")
			{
				$('#country_msg').html("Your country location is required");
			}
		}
		else
		{
				$.ajax({
					url:"handler/update_com_info.php",
					type:"POST",
	...
	$('#logo_form').submit(function(e){
		...
		if(logo=="")
		{
			$('#logo_msg').html("A company logo is required");
		}
		else
		{
				$.ajax({
					url:"handler/change_logo.php",
					type:"POST",
					data: new FormData(this),
					cache:false,
					contentType:false,
					...
					success:function(data)
					{
						...
						{
							var id = data.comid;
							$.post("handler/get_com_logo.php",{id:id},function(response){
								$('#paste_logo').html(response);
							})
						}
					}
				})
