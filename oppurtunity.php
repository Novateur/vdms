...
<?php echo "<body onload=\"get_opp_grid()\">"; ?>
                ...
                <section class="scrollable padder"><br/>              
					<div class="col-sm-12">
					  ...
					  <div class='pull-right'>
						<a href='#' onclick="get_opp_grid()"><i class='i i-grid'></i></a> <a href='#' onclick="get_opp_list()"><i class='fa fa-align-justify'></i></a>
					  </div>
                    </div>
					<div class="col-sm-12">
						<div id="paste_opp">
  ...
  <script>
	function get_opp_grid()
	{
		$.get("handler/opp_grid_all.php?page=1&type=grid",function(response){
			$('#paste_opp').html(response);
		})
	}
	...
	function get_opp_list()
	{
		$.get("handler/opp_list_all.php?page=1&type='list'",function(response){
			$('#paste_opp').html(response);
		})
	}
	function my_tender()
	{
		$.get("handler/opp_grid_filt.php?page=1&type='grid'",function(response){
			$('#paste_opp').html(response);
			$('#choose_task').html("View my tenders <i class='i i-arrow-down4'></i>");
		})
	}	
	function my_tender_list()
	{
		$.get("handler/opp_list_filt.php?page=1&type='list'",function(response){
			$('#paste_opp').html(response);
			$('#choose_task').html("View my tenders <i class='i i-arrow-down4'></i>");
		})
	}
	function paginate(page_to,page,type)
	{
		if(type=='grid')
		{
			$.get("handler/opp_grid_all.php?page="+page_to+"&type='"+type+"'",function(response){
				$('#paste_opp').html(response);
				//$('#choose_task').html("View my tenders <i class='i i-arrow-down4'></i>");
			})
		}		
		if(type=='list')
		{
			$.get("handler/opp_list_all.php?page="+page_to+"&type='"+type+"'",function(response){
				$('#paste_opp').html(response);
			})
		}
	}	
	function paginate_filt(page_to,page,type)
	{
		if(type=='grid')
		{
			$.get("handler/opp_grid_filt.php?page="+page_to+"&type='"+type+"'",function(response){
				$('#paste_opp').html(response);
				$('#choose_task').html("View my tenders <i class='i i-arrow-down4'></i>");
			})
		}		
		if(type=='list')
		{
			$.get("handler/opp_list_filt.php?page="+page_to+"&type='"+type+"'",function(response){
				$('#paste_opp').html(response);
				$('#choose_task').html("View my tenders <i class='i i-arrow-down4'></i>");
			})
		}
	}
	function del_opp(id,page,type)
	{
			$('#paste_proceed_btn').html("<button class='btn btn-danger' id='proceed_del' value='"+id+"'onclick=\"cont_del_opp(this.value,'"+type+"','"+page+"')\">Proceed</button>");
			$('#delete_oppurtunity').modal('show');
	}	
	function del_opp_filt(id,page,type)
	{
			$('#paste_proceed_btn').html("<button class='btn btn-danger' id='proceed_del' value='"+id+"'onclick=\"cont_del_opp_filt(this.value,'"+type+"','"+page+"')\">Proceed</button>");
			$('#delete_oppurtunity').modal('show');
	}
	function cont_del_opp(id,type,page_to)
	{
		$.post("handler/delete_oppurtunity.php",{id:id,type:type},function(response){
			if(response.status=="deleted" && response.type=="list")
			{
				$.get("handler/opp_list_all.php?page="+page_to+"&type='"+type+"'",function(response){
					$('#delete_oppurtunity').modal('hide');
					$('#paste_opp').html(response);
					//$('#choose_sort').html("Sequence ascending <i class='i i-arrow-down4'></i>");
				})
			}			
			if(response.status=="deleted" && response.type=="grid")
			{
				$.get("handler/opp_grid_all.php?page="+page_to+"&type='"+type+"'",function(response){
					$('#delete_oppurtunity').modal('hide');
					$('#paste_opp').html(response);
	...
	function cont_del_opp_filt(id,type,page_to)
	{
		$.post("handler/delete_oppurtunity.php",{id:id,type:type},function(response){
			if(response.status=="deleted" && response.type=="list")
			{
	...
	{
		var favorite=[];
		$.each($("input[name='opps[]']:checked"),function(){
	...
	function del_multi(page,type)
	{
		$('#paste_multi_proceed_btn').html("<button class='btn btn-danger' id='proceed_del' onclick=\"cont_del_multi("+page+",'"+type+"')\">Proceed</button>");
		$('#delete_multi').modal('show');
	}
	function cont_del_multi(page,type)
	{
		$('#delete_multi').modal('hide');
		$.ajax
			({
				url:"handler/multi_del_opp.php",
				type:"POST",
				...
				success:function(response)
				{
					if(response=="deleted")
					{
						$.get("handler/opp_list_filt.php?page="+page+"&type='"+type+"'",function(response){
							$('#delete_multi').modal('hide');
							$('#paste_opp').html(response);
							$('#choose_task').html("View my tenders <i class='i i-arrow-down4'></i>");
						})
