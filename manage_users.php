...
echo "<body onload=\"get_details({$id})\">";
?>
  <section class="vbox">
      ...
      <section class="hbox stretch">
                  ...
                  <ul class="nav nav-main" data-ride="collapse">
					...
					<li class="active">
                      ...
                      <ul class="nav dk">
                        ...
                        <li class="active">
                          <a href="manage_users.php" class="auto">                                                        
                            <i class="i i-dot"></i>
        ...
        <section id="content">
              ...
              <section class="vbox">
                <section class="scrollable padder">              
                  <section class="row m-b-md">
                    ...
                    <div style="margin-left:20px;margin-top:-50px" class="col-sm-10"><br/>
						<section class="panel panel-default">
							<header class='panel-heading font-bold'>
								<h4>Added users</h4>
							</header>
							<div class='panel-body' style="min-height:400px;max-height:500px;overflow:auto">
								<div style="margin-top:-35px;max-height:170px;overflow:auto">
								<?php
								
									get_company_users();	
								?>
								</div><br/>
								<div class="row" id="paste_details">
							...
							<header class='panel-heading font-bold' style='padding:30px'>
								<div class='form-group' style='margin-top:-10px;float:right'>
									<div class='col-sm-4 col-sm-offset-2'>
										<a href='settings.php' data-role='button' class='btn btn-success'>Done</a>
									</div>
								</div>
							</header>
						</section>
          ...
          <a href="#" class="hide nav-off-screen-block" data-toggle="class:nav-off-screen" data-target="#nav"></a>
    		...
    		<div id="delete_modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"data-backdrop="static" data-keyboard="false">
				<div class="modal-dialog" style="width:400px;">
					<div class="modal-content">
						...
						<div class="modal-body" style='font-size:14px'>
							Are you sure you want to delete this user,once you proceed with this action there is no going back
						</div>
						<div class="modal-footer">
							<button class="btn btn-default" style='border:1px solid #357ebd' data-dismiss="modal" aria-hidden="true">Cancel</button>
							<button id="yes_con" class="btn btn-danger" onclick="cont_del(this.value)">Proceed</button>
						</div>
					</div>
				</div>
			</div>
  ...
  <script>
	...
	{
		$.post("handler/get_user_details.php",{id:id},function(response){
			$('#paste_details').html(response);
		})
	}
	...
	{
		$.post("handler/delete_user.php",{id:id},function(response){
			//alert(response);
			if(response=="deleted")
			{
	...
	function save_user_edit(id)
	{
		...
		if(name=="" || email=="" || position=="")
		{
			if(name=="")
			{
				$('#name_msg').html("A user's name is required");
			}			
			if(email=="")
			{
				$('#email_msg').html("A user's email is required");
			}			
			if(position=="")
			{
				$('#position_msg').html("A user's position is required");
			}
		}
		else
		{
			$.post("handler/edit_user.php",{id:id,name:name,email:email,position:position,pass:pass,access:access},function(response){
				if(response=="updated")
				{
					location.reload(true);
				}
	...
	{
		$.get("handler/invite_user.php",function(response){
			$('#paste_details').html(response);
		})
	}
	function send_invite()
	{
		...
		var access = $('#access').val();
		//var pass = $('#pass').val();
		if(name=="" || email=="" || position=="" || access=="")
		{
			if(name=="")
			{
				$('#name_msg').html("User's name is required");
			}			
			if(email=="")
			{
				$('#email_msg').html("User's email is required");
			}			
			if(position=="")
			{
				$('#position_msg').html("User's position is required");
			}			
			if(access=="")
			{
				$('#access_msg').html("User's access level is required");
			}
		}
		else
		{
			$('#send_invite').html("<img src='loaders/loading.gif'/>");
			$.post("handler/send_invite.php",{name:name,email:email,position:position,access:access},function(response){
				if(response=="sent")
				{
					location.reload(true);
				}
